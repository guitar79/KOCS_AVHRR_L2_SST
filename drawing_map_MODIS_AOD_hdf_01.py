...
if shape(longitude) != shape(latitude) or shape(latitude) != shape(aod) :
    print('data validation error')
else : 
    for i in range(shape(longitude)[0]) :
        for j in range(shape(longitude)[1]) :
            for ii in range(array_data.shape[0]) :
                for jj in range(array_data.shape[1]) :
                    if array_data[ii][jj]['latitude'] == latitude[i][j] \
                    and array_data[ii][jj]['longitude'] == longitude[i][j] :
                        array_data[ii][jj]['data'].append(aod[i][j])

PRO HDF_SD_VARREAD, HDFID, VARNAME, ScaleFactor, MinValid, MaxValid, DATA, _EXTRA=EXTRA_KEYWORDS
...
;- Check arguments
if (n_params() ne 6) then message, 'Usage: HDF_SD_VARREAD, HDFID, VARNAME, DATA'
...
bad_value=where((data LT MinValid) or (data GT MaxValid))

data = temporary(data)*ScaleFactor

if (bad_value(0) ne -1) then data(bad_value) = !values.f_nan
